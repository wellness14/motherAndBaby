<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0" />
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	<link rel="stylesheet" type="text/css" href="/etc/store_member/css/style_hong.css">
    <title>마더앤베이비모바일</title>




</head>



<body>
	<div id="header">
<div id="navi_tit_name"><span  class="position_navi">통계 > 업체 > 고객평가확인</span>
</div>
		
	<hr class="tab_line">
	
		<div class="tit_name"> <SPAN>조리원 ASP </span></div>





<div id="line_bar">
	<img src="/etc/store_member/img/line_b.png" style="" />
</div>



</div>

	



<table width="770"  align="center" border="1" cellspacing="2" cellpadding="1"id="member_content_02">

	<tr>
		<th class="content_tit">응답율 순위</th>
		<th class="content_tit">업체명</th>
		<th class="content_tit">평균별 수</th>
		<th class="content_tit">누적점수</th>
		<th class="content_tit">고객참여건수(건)</th>
		<th class="content_tit">관리</th>
	</tr>

	<tr>
		<td class="content"> 1</td>
		<td class="content"> 헤리티지</td>
		<td class="content"> ★★★★☆</td>
		<td class="content"> 304,020</td>
		<td class="content"> 301</td>
		<td class="content"><span class="setting" >관리</span></td>
	</tr>

	<tr>
		<td class="content"> 1</td>
		<td class="content"> 궁조리원</td>
		<td class="content"> ★★★★☆</td>
		<td class="content"> 304,020</td>
		<td class="content"> 301</td>
		<td class="content"><span class="setting" >관리</span></td>
	</tr>

	





</table>








<div class="page_num">
				<ul><li><a href="#"><img src="/etc/image/page_left.png" style="width:8px;"/></a>
					<li> <a href="#">1</a> </li>
					<li> <a href="#">2</a> </li>
					<li> <a href="#">3</a> </li>
					<li> <a href="#">4</a> </li>
					<li> <a href="#">5</a> </li>
					<li> <a href="#">6</a> </li>
					<li> <a href="#">7</a> </li>
					<li> <a href="#">8</a> </li>
					<li> <a href="#">9</a> </li>
					<li><a href="#"><img src="/etc/image/page_right.png"style="width:8px;"/></a>
				</ul>
			</div>



</div>



</div>


</body>

</html>
