...
Response.ContentType="text/html;charset=utf-8"
	...
	'strSql = strSql & "			* "
	'strTable = "    from MO_HELPER_SERVICE a, MO_RESERVATION b , MO_HELPER c   "
	'strWhere = "  where a.MR_IDX = b.MR_IDX   and a.MD_HELPER_SEQ = c.MD_HELPER_SEQ  and c.MD_HELPER_ID = 'test' "
  ...
  <head>
	...
	<script type="text/javascript" language="utf-8">
		...
		$(document).ready(function() {
			...
			//	alert(modal_serach);
				if(modal_serach != ""){
					window.open("http://mothernbaby.co.kr/mnb_notebook/customer_list.asp?search="+modal_serach, "_self","");
				}
			});
...
%>
						<tr class="warning">
							<td><a href="customer_view.asp?idx=<%=idx%>"><%=Left(s_date,4)%>.<%=Mid(s_date,5,2)%>.<%=Right(s_date,2)%></a></td>
							<td><a href="customer_view.asp?idx=<%=idx%>"><%=name%></a></td>
							<td><% If pay = "결제완료" And s_date <=  nowymd   And e_date >= nowymd then%> 진행중 <% End if%>
								<% If pay = "결제완료" And  e_date < nowymd then%> 완료 <% End if%>
								<% If pay = "" Then %> 대기중 <%else%>대기중<% End if%>
							</td>
						</tr>
					</tbody>
<%
