//변수 선언 및 초기화
		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					//bSupportDragAndDropAPI = true;	//수정한곳 크롬에서 드래그앤 드롭이 안되는건 어차피 욕먹으므로 강제로 false 시켜버림
					bSupportDragAndDropAPI = false;
				}
			} else {
				bSupportDragAndDropAPI = false;
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    	     welBtnConfirm.attr("src", "/common/SmartEditor2/img/photoQuickPopup/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    		 ...
    		 welBtnConfirm.attr("src", "/common/SmartEditor2/img/photoQuickPopup/btn_confirm.png");
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			...
			aFileList.push('	<a onclick="delImage(\'img' + nImageInfoCnt + '\')"><img class="del_button" src="/common/SmartEditor2/img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
    	...
    	sUploadURL = fileUrl; 	//upload URL	//수정한곳 도메인 빼버림, html5모드 작동 소스 없으나, 향후 능력자님이 만들면 주소 이렇게 하실듯..
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 		    sUrl: fileUrl,	//샘플 URL입니다.	//수정한곳 도메인 빼버림
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
    ...
    function closeWindow(){
	   	...
	   	window.close();		//수정한곳 원본에선 여기 주석처리되어있었는데 주석해제함
    }
