 ...
 * Release:  1.05 (4/10/2008) Fix to dragMask (now an img)

                             'hostMIF' reference is now added to the embedded window object providing
                              a reference to the MIF object that hosts it(same domain only).
...
Ext.extend(Ext.ux.ManagedIFrame , Ext.util.Observable,
  ...
  * Version:  1.05  (4/10/2008) Added:
                                   - new Element visibility method to prevent iframes,object,embeds from
                                       refreshing during DOM reflow.
                                   - shimCls, shimUrl (you can shim a frame with an actual image)
                                   - New hideMode added : 'nosize' to augment 'display' and 'visibility' ( hidemode
                                     is defaulted based on current browser (!Ext.isIE ? 'nosize' : 'display' )
 ...
 Ext.ux.ManagedIframePanel = Ext.extend(Ext.Panel, {
   ...
   ,initComponent : function(){
    ...
    onRender : function(ct, position){
        ...
        this.shim = Ext.get(this.body.child('.x-frame-shim'));
    },
    toggleShim   : function(){

        if(this.shim && this.shimCls)this.shim.toggleClass(this.shimCls);
    },
        // private
