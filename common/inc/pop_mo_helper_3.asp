...
<%
	  ...
	  mo_helper_name1		= Trim(fncRequest("mo_helper_name_3"))
...
'	strWhere = "    "
	strTable_cnt = strTable_cnt & " FROM MO_HELPER  A  LEFT OUTER JOIN MO_HELPER_SERVICE B  ON A.MD_HELPER_SEQ = B.MD_HELPER_SEQ  LEFT OUTER JOIN MO_RESERVATION C  ON B.MR_IDX = C.MR_IDX where MD_HELPER_NAME like '%"& mo_helper_name1 &"%' and  MD_HELPER_LOCAL='"& grpcd &"'"
%>
<!--
	strTable = strTable & " INNER JOIN MO_MEM_DTL D WITH(NOLOCK) ON D.MM_MEM_ID = D.MD_MEM_ID "
...
SELECT * FROM (
SELECT  *, ROW_NUMBER( ) OVER (ORDER BY  MD_HELPER_NAME ASC) as ROWNUM1  
 FROM 	 
 (  SELECT	
   A.MD_HELPER_NAME
   ,C.MR_MEM_NM				      
	 ...
	 ,C.MR_SERVICEEDT			      
	  ,B.MD_HELPER_COMMENT			  
	   ,A.MD_HELPER_SEQ		      	 	
		 ...
		 FROM	MO_HELPER  A  LEFT OUTER JOIN MO_HELPER_SERVICE B  ON A.MD_HELPER_SEQ = B.MD_HELPER_SEQ 
		                      LEFT OUTER JOIN MO_RESERVATION C  ON 
		 B.MR_IDX = C.MR_IDX  
		 
		 ) T1  WHERE   MD_HELPER_NAME LIKE '%오%') T2 WHERE ROWNUM1 > 0 AND ROWNUM1 <= 10 ORDER BY  MD_HELPER_NAME ASC
...
<script type="text/javascript">
	...
	function fncInputAddr(postcd1,postcd2,hp_Seq){
		opener.InputName_Comment_3(postcd1,postcd2,hp_Seq);
		self.close();
	}
