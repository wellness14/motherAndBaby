...
NowDay = Day(Date())    '8
	...
	If NowYear <> "" And NowMonth <> "" And NowDay <> "" Then 
	yyyymmdd = NowYear & fncSetNumber(NowMonth) & fncSetNumber(NowDay)
	End If 

	If yyyymmdd <> "" Then 
		now_m_date = Mid(yyyymmdd,5,2)
		If now_m_date < 10 Then now_m_date = Right(now_m_date,1)
		now_d_date = Right(yyyymmdd,2)
		If now_d_date < 10 Then now_d_date = Right(now_d_date,1)
	End If 
...
<%
	...
	If now_seq <> "" Then
		...
		strTable = " FROM MO_RESERVATION_GIFT_NOW_EA WITH(NOLOCK) "
	 
		strWhere = " WHERE MD_GIFT_NOW_EA_SEQ = '"& now_seq &"' and MD_GIFT_SEQ =  '"& pdt_seq &"' "
		...
		strSql = strSql & "			,ROW_NUMBER( ) OVER (ORDER BY MD_GIFT_NOW_EA_SEQ DESC) ROWNUM "
...
'-----------------------------------------------------------------------------
	...
	If in_date <> "" Then 
		Min_date = Mid(in_date,5,2)
		If Min_date < 10 Then Min_date = Right(Min_date,1)
		Din_date = Right(in_date,2)
		If Din_date < 10 Then Din_date = Right(Din_date,1)
	End If 
%>
...
<body>
	<div id="container">
		...
		<div class="write">
			...
			<input type="hidden" name="now_seq" value="<%=now_seq%>">
			...
			<tr>
				...
				<td>
					...
					<% For num=Year(DateAdd("yyyy",-1,Date())) To Year(DateAdd("yyyy",3,Date())) %>
						<option value="<%=num%>" <%=fncSetSelectBox(num,Left(in_date,4))%>><%=num%></option>
					<% Next %>
					</select>
					<select name="in_m">
						<option value="">월</option>
					<% For num=1 To 12 %>
						<option value="<%=num%>" <%=fncSetSelectBox(num,Min_date)%>><%=num%></option>
					<% Next %>
					</select>
					<select name="in_d">
						<option value="">일</option>
					<% For num=1 To 31 %>
						<option value="<%=num%>" <%=fncSetSelectBox(num,Din_date)%>><%=num%></option>
					<% Next %>
					</select>
				</td>
			</tr>
			<tr>
				<th>입고수량</th>
				<td>
					<input type="text" id="in_ea" name="in_ea" size="20" value="<%=arrList(3,0)%>" valid="E|M=10|A=수량을 입력해주세요">개
				</td>
			</tr>
		...
		<div class="write">
			...
			<tr>
				...
				<td>
					...
					<select name="in_m">
						<option value="<%=now_m_date%>"><%=now_m_date%>월</option>
					<% For num=1 To 12 %>
						<option value="<%=num%>" <%=fncSetSelectBox(num,Min_date)%>><%=num%></option>
					<% Next %>
					</select>
					<select name="in_d">
						<option value="<%=now_d_date%>"><%=now_d_date%>일</option>
					<% For num=1 To 31 %>
						<option value="<%=num%>" <%=fncSetSelectBox(num,Din_date)%>><%=num%></option>
					<% Next %>
					</select>
			...
			<tr>
				<th>입고수량</th>
				<td>
					<input type="text" id="in_ea" name="in_ea" size="20" value="" valid="E|M=10|A=수량을 입력해주세요">개
				</td>
			</tr>
...
<script type="text/javascript">
	...
	btnDel.click(function(){
		if (confirm("삭제 하시겠습니까?")) {
			jQuery('#ifrm').attr({'src':'./reservation_gift_now_sql.asp?mode=delete&now_seq=<%=now_seq%>&pdt_seq=<%=pdt_seq%>'}); 
		}
	});
	...
	frm.submit(function(){
		...
		"target" : "ifrm",
		"action" : "./reservation_gift_now_sql.asp"
	});
