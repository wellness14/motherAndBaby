...
'---------------------------------------------------------
	...
	strWhere = " WHERE MR_CNLKBN='N'  and ((SELECT CONVERT(varchar(30), GETDATE(), 112)) >= MR_SERVICESDT-2) and ( (SELECT CONVERT(varchar(30), GETDATE(), 112)) <= MR_SERVICEEDT-2)    "
	...
	strSql = strSql & "		SELECT "
	strSql = strSql & "			* "
	strSql = strSql & "			,ROW_NUMBER( ) OVER (order by  MR_SERVICESDT desc, MR_SERVICEEDT asc) ROWNUM "
	strSql = strSql & strTable
	strSql = strSql & strWhere
	strSql = strSql & "  ) T1 "
	strSql = strSql & " WHERE  "
...
<body>
	<div id="container">
		<div class="linemap">마더앤베이비 > 회원관리 > <strong>해피콜 리스트</strong></div>
		<div class="write">
...
%>
			<tr onclick="location.href='./member_reser_ins.asp?idx=<%=idx%>'" style="cursor:pointer;">
...
<%If service_sdt <> "" then %>
				...
				<td><%=Left(service_edt,4)%>-<%=Mid(service_edt,5,2)%>-<%=Right(service_edt,2)%></td>
<%else%>        <td></td>  <% End if%>
				<td></td>
				<td></td>
