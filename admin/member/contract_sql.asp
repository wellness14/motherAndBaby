...
'	Request
	...
	If mode = "ins" Or mode = "upd" Then 
		...
		constract_date				= fncRequest("constract_date")				' 생일(달력)  
		start_date					= fncRequest("start_date")				' 생일(달력)  
		end_date					= fncRequest("end_date")				' 생일(달력)  
...
'--------달력추가로 수정 ----------------------------------------------------------
	If constract_date <> "" then
		constract_date_arr=split(constract_date,"-") 
		constract_date_y = constract_date_arr(0)
		constract_date_m = constract_date_arr(1)
		constract_date_d = constract_date_arr(2)
		constract_date_ymd = constract_date_y &  constract_date_m  &  constract_date_d 
	End If
	
	If start_date <> "" then
		start_date_arr=split(start_date,"-") 
		start_date_y = start_date_arr(0)
		start_date_m = start_date_arr(1)
		start_date_d = start_date_arr(2)
		start_date_ymd = start_date_y &  start_date_m  &  start_date_d 
	End If
	
	If end_date <> "" then
		end_date_arr=split(end_date,"-") 
		end_date_y = end_date_arr(0)
		end_date_m = end_date_arr(1)
		end_date_d = end_date_arr(2)
		end_date_ymd = end_date_y &  end_date_m  &  end_date_d 
	End If
	...
	Else 
		...
		Select Case mode 
			Case "ins" : 
				...
				strSql = strSql & ",      MO_CONTRACT_LOCAL "
				strSql = strSql & ",      MO_CONTRACT_LOCAL_DETAIL "
				strSql = strSql & " )  "
				...
				strSql = strSql & "	,		'"& constract_date_ymd &"' "
				strSql = strSql & "	,		'"& tel &"'   "
				strSql = strSql & "	,		'"& imda_type &"' "
				strSql = strSql & "	,		'"& bank &"'  "
				strSql = strSql & "	,		'"& account &"' "
				strSql = strSql & "	,		'"& start_date_ymd &"'   "
				strSql = strSql & "	,		'"& end_date_ymd &"' "
...
' select ISNULL(MAX(MO_EXTEND),0)  FROM MO_HELPER_SERVICE   where  MD_HELPER_SEQ = '169' 도우미 시퀀스로 연장횟수 구하기
				...
				Case "upd" : 
					...
					strSql = strSql & "      MO_CONTRACT_NAME		= '"& company &"' "
					strSql = strSql & ",      MO_CONTRACT_POST		= '"& post_company &"' "
					strSql = strSql & ",      MO_CONTRACT_ADDR1		= '"& addr_11 &"' "
					strSql = strSql & ",      MO_CONTRACT_ADDR2		= '"& addr_22 &"' "
					strSql = strSql & ",      MO_CONTRACT_HP		= '"& hp_company &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_ID    = '"& mem_id &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_NAME  = '"& mem_nm &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_POST  = '"& post &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_ADDR1 = '"& addr_1 &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_ADDR2 = '"& addr_2 &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_HP    = '"& hp &"' "
					strSql = strSql & ",      MO_CONTRACT_PRODUCT_NAME   = '"& contract_product &"' "
					strSql = strSql & ",      MO_CONTRACT_DATE			= '"& constract_date_ymd &"' "
					strSql = strSql & ",      MO_CONTRACT_TEL			= '"& tel &"' "
					strSql = strSql & ",      MO_CONTRACT_TYPE			= '"& imda_type &"' "
					strSql = strSql & ",      MO_CONTRACT_BANK			= '"& bank &"' "
					strSql = strSql & ",      MO_CONTRACT_ACCOUNT		= '"& account &"' "
					strSql = strSql & ",      MO_CONTRACT_START_DATE    = '"& start_date_ymd &"' "
					strSql = strSql & ",      MO_CONTRACT_END_DATE		= '"& end_date_ymd &"' "
					strSql = strSql & ",      MO_CONTRACT_PRODUCT_NO	 = '"& product_no &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_MAIN  = '"& contract_in_name &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_1		= '"& contract_1_term &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_2		= '"& contract_2 &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_3		= '"& contract_3 &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_4_PRICE  = '"& contract_11_pay &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_4_DEPOSIT  = '"& contract_11_deposit &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_4_SET  = '"& contract_11_set &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_4_OUT_PAY  = '"& contract_11_out &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_4_TOTAL_PAY  = '"& contract_11_total &"' "
					strSql = strSql & ",      MO_CONTRACT_CONTENT_HUMAN  = '"& contract_11_pay_man &"' "
					strSql = strSql & ",      MO_CONTRACT_NOW_DATE  = '"& contract_11_date &"' "
					strSql = strSql & ",      MO_CONTRACT_CUSTOMER_NAME2  = '"& contract_11_name &"' "
					strSql = strSql & ",      MO_CONTRACT_LOCAL  = '"& class_value_1 &"' "
					strSql = strSql & ",      MO_CONTRACT_LOCAL_DETAIL  = '"& title_value_1 &"' "
...
<script type="text/javascript">
	function Init(e) {
		switch(e) {
			...
			case(2) :
				alert('정상적으로 등록되었습니다.');
				parent.location.href = "./contract_list.asp?<%=strParam%>"
			break;
			case(3) :
				alert('수정 등록되었습니다.');
				parent.location.href = "./contract_list.asp"
			break;
			case(4) :
				alert('삭제 되었습니다.');
				parent.location.href = "./contract_list.asp"
