...
'---------------------------------------------------------
	...
	strTable = " FROM MO_CONTRACT A WITH(NOLOCK) "
	strTable = strTable &  ""
	strWhere = ""


'SELECT COUNT(1) FROM MO_CONTRACT A WITH(NOLOCK) AND MD_LOCAL = '1010' 
	'strTable = strTable & " INNER JOIN MO_MEM_DTL WITH(NOLOCK) ON MR_MEM_ID = MD_MEM_ID "
	'strWhere = " WHERE MR_CNLKBN='N'   "

	If grpcd <> "" And grpcd <> "1" Then strWhere = strWhere & " where MO_CONTRACT_LOCAL = '"& grpcd &"' "
	If sword <> "" Then 
		Select Case smode
			Case "1" : strWhere = strWhere & "  where MO_CONTRACT_CUSTOMER_ID like '%"& sword &"%' "
			Case "2" : strWhere = strWhere & "  where MO_CONTRACT_CUSTOMER_NAME like '%"& sword &"%' "
		End Select 
	End If 
%>
...
<%
	...
	strSql = strSql & "		SELECT "
	strSql = strSql & "			* "
	strSql = strSql & "			,ROW_NUMBER( ) OVER (ORDER BY MO_CONTRACT_SEQ DESC) ROWNUM "
	strSql = strSql & strTable
	strSql = strSql & strWhere
	strSql = strSql & "  ) T1 "
	strSql = strSql & " WHERE  "
...
<body>
	<div id="container">
		<div class="linemap">마더앤베이비 > 회원관리 > <strong>임대차 계약서 리스트</strong></div>
		<div class="write">
			...
			<div class="selection">
				...
				<a onClick="location.href='./contract_list.asp';" style="cursor:pointer;" class="button">전체 리스트</a>
				<span style="margin-left:20px;">
					<a onClick="location.href='./contract_ins.asp'" style="cursor:pointer;" class="button">등록</a>
				</span>
			</div>
			</form>
			<br />
...
<%
	If IsArray(arrList) Then 
		For num=0 To UBound(arrList,2)		
			idx			= arrList(0,num)
			mem_id		= arrList(6,num)
			mem_nm		= arrList(7,num)
			product		= arrList(12,num)
			product_no		= arrList(20,num)
			...
			start_date	= arrList(19,num)
			end_date	= arrList(18,num)
 



%>
			<tr onclick="location.href='./contract_ins.asp?idx=<%=idx%>'" style="cursor:pointer;">
				<td><%=intTotalCnt - CInt(page-1) * page_size - num%></td>
				<td><%=mem_nm%><br><%If mem_id <> "" Then%>(<%=mem_id%>)<%End If%></td>
				<td><%=product%></td>
				<td><%=product_no%> </td>
				<td><%=Left(start_date,4)%>-<%=Mid(start_date,5,2)%>-<%=Right(start_date,2)%></td>
				<td><%=Left(end_date,4)%>-<%=Mid(end_date,5,2)%>-<%=Right(end_date,2)%></td>
			</tr>
<%
		Next 
	Else
