...
'---------------------------------------------------------
	...
	strWhere = strWhere & " and  ( (SELECT CONVERT(varchar(30), GETDATE(), 112))  BETWEEN  MR_SERVICESDT   and    MR_SERVICEEDT and MR_PAY_STATE = '결제완료' or  (SELECT CONVERT(varchar(30), GETDATE(), 112))  BETWEEN  MR_SERVICESDT1     and    MR_SERVICEEDT1 or  (SELECT CONVERT(varchar(30), GETDATE(), 112))  BETWEEN  MR_SERVICESDT2     and    MR_SERVICEEDT2 or  (SELECT CONVERT(varchar(30), GETDATE(), 112))  BETWEEN  MR_SERVICESDT3     and    MR_SERVICEEDT3 and (MR_CANCEL_YN = 'N' or MR_CANCEL_YN ='' or  MR_CANCEL_YN is null ) )"
...
<%
	If IsArray(arrList) Then 
		For num=0 To UBound(arrList,2)		
			...
			End If
%>
			<tr onclick="location.href='./member_reser_ins.asp?idx=<%=idx%>&listKind=ing'" style="cursor:pointer;background:<%=trBackground%>;">
