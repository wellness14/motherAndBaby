...
'	com_info	= fncRequestCookie("com_info")
	...
	If USERID <> "" And USER_PWD <> "" Then   '임시 아이디   AND LEN(com_info)=35
		strSql = "SELECT UL_MEM_NM, UL_GRP_CD, UL_LEVEL, UL_GRP_CD2 FROM MO_USER_LIST with(nolock) WHERE UL_MEM_ID = '" & USERID & "' AND UL_USE_YN ='Y'  AND UL_PWD = dbo.UF_MD5('"& USER_PWD &"') "
		...
		If Not Rs.EOF Then 
			...
			Response.Cookies("userlevel") = TRIM(RS(2))
			Response.Cookies("userlevel").Path = "/"
			Response.Cookies("userlevel").Domain = strDomain

			Response.Cookies("usergrpcd2") = TRIM(RS(3))
			Response.Cookies("usergrpcd2").Path = "/"
			Response.Cookies("usergrpcd2").Domain = strDomain
...
<!--
   ...
   window.top.location.href="/admin/index.asp"
//-->
</SCRIPT>