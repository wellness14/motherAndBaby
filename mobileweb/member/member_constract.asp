...
<%
	...
	If uId <> "" Then 
		...
		strTable = strTable & "   "
		strWhere = " where  MO_CONTRACT_CUSTOMER_ID  = '"& uId &"'"
		...
		If Not Rs.EOF Then 
			...
			arrList = Rs.getRows()
		Else 
			response.Write "<script type='text/javascript'>alert('임대차 계약정보가 없습니다.');location.href='http://mothernbaby.co.kr/mobileweb/main/';</script>"
			response.End 
		End If 
		Call RSClose(Rs)
	Else 
...
<body>
	  <div id="mcontainer">
			<table width="360" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					<td width="360" style="padding-top:40px;text-align:center;"> <h1>임 대 차 계 약 서</h1><!--<img src="/image/sub06_subtitle05.jpg" width="770" height="41" />--></td>
				</tr>
				<tr>
			...
			<div style="margin-left:20px;width:360px;">
			   임대인 : <%=company_name%>   <br/> 계약물품 : <%=product_name%>   <br/>
			   주 소 : <%=company_post%>&nbsp; <%=company_addr1%>&nbsp;<%=company_addr2%><br/>연락처 : <%=company_hp%>  <Br/>계약일 : <%=constract_date%> <br/>
			   임차인 : <input type="text" id="mem_nm" name="mem_nm" size="20" value="<%=customer_nm%>"><br/>연락처 : <%=customer_hp%><br/>전화 : <%=constract_tel%></span><br/>
			   주 소 : <%=customer_post%>&nbsp; <%=customer_addr1%>&nbsp; <%=customer_addr2%> <br/>임대경유 : <%=constract_type%> <br/>
			   <strong><font style="font-weight: bold">사용인 관련자료 : </font></strong> 대여개시일 : <%=Left(start_date,4)%>-<%=Mid(start_date,5,2)%>-<%=Right(start_date,2)%><br/>
				<span style="margin-left:132px;">대여종료일 : <%=Left(end_date,4)%>-<%=Mid(end_date,5,2)%>-<%=Right(end_date,2)%></span> <br/>입금계좌 : <%=constract_account%>(<%=constract_bank%>) <br/>
		 ...
		 <br/>
			<div style="margin-left:20px;">
				...
				전항에 의한 계약이 해약 및 임대 기간의 만기 등에 의해 종료되었을 경우에는 임차인은 지체없이 제품을 임대인에게 반환한다. 임차인이 특정한 위치까지 제품의 배달 또는 반납 목적으로 본 회사 직원의 방문을 요구하여 본사직원이 특정위치까지 방문 할 경우 방문 수수료가 추가된다.<br/>(유축기 배달 및 반납 요청시 방문수수료 : 서울시 편도 5,000원, 경기도 편도 10,000원)<br/><br/>
				...
				대&nbsp; 여&nbsp;&nbsp; 료 :  <span style="margin-left:10px;"></span><input type="text" id="contract_11_pay" name="contract_11_pay" size="10" value="<%=contnent4_price%>" disabled>  <br/>
				보&nbsp; 증&nbsp;&nbsp; 금 :  <span style="margin-left:10px;"></span><input type="text" id="contract_11_deposit" name="contract_11_deposit" size="10" value="<%=contnent4_deposit%>" disabled>  <br/>
				소모품세트 : <input type="text" id="contract_11_set" name="contract_11_set" size="10" value="<%=contnent4_set%>" disabled>  <br/>
				출&nbsp; 장&nbsp;&nbsp; 료 :  <span style="margin-left:10px;"></span><input type="text" id="contract_11_out" name="contract_11_out" size="10" value="<%=contnent4_out_pay%>" disabled> <br/>
				합 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp; 계 :  <span style="margin-left:5px;"></span><input type="text" id="contract_11_total" name="contract_11_total" size="10" value="<%=contnent4_total%>" disabled> <br/><span style="margin-left:0px;">영&nbsp;&nbsp; 수&nbsp;&nbsp; 자 <span style="margin-left:10px;"></span><input type="text" id="contract_11_pay_man" name="contract_11_pay_man" size="10" value="<%=contnent4_human%>" disabled></span><br/>
			...
			<span style="margin-left:20px;">임차인 성명<input type="text" id="contract_11_name" name="contract_11_name" size="10" value="<%=customer_nm2%>"></span><br/>
			</div>

		</form>
		<br/>	
					<div align="center">
					<input type="button" width="150" height="40" border="0" id="btnSubmit" style="cursor:pointer;" value="확인"/> &nbsp;&nbsp;
					<input type="button" width="150" height="40" border="0" onclick="location.reload();" style="cursor:pointer;" value="취소"/> 
