	...
	Else 
		Select Case mode 
			Case "ins" : 
				...
				strSql = strSql & "         MD_EMAIL,MD_SNS_FLG,MD_EMAIL_FLG,MD_LOCAL,MD_LOCAL_DETAIL,MD_CHILDBIRTH,MD_CHILD_FLG,MD_TYPE, MD_HP_ADD "
				strSql = strSql & " )  "
				strSql = strSql & " SELECT "
				strSql = strSql & "			'"& mem_id &"','"& phone &"','"& hp &"','"& radio1 &"','"& birth &"','"& post &"','"& addr_1 &"','"& addr_2 &"', "
				strSql = strSql & "			'"& email &"','"& check3 &"','"& check4 &"','"& class_value_1 &"','"& title_value_1 &"','"& childbirth &"','"& radio3 &"','"& radio4 &"','"& add_phone &"'; "
...
<script type="text/javascript">
	function Init(e) {
		switch(e) {
			...
			case(2) :
				alert('정상적으로 등록되었습니다.');
				parent.location.href = "/board/board_01_view.asp?idx=41&brd_id=EVENT&sec_flg=Y&mOne=4&mTwo=5"
			break;
			case(3) :
				alert('수정 등록되었습니다.');
				parent.location.href = "/board/board_01_view.asp?idx=41&brd_id=EVENT&sec_flg=Y&mOne=4&mTwo=5"
