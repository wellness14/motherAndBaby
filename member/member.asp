...
<!--					<tr style="display:none;">
					...
					<tr>
						...
						<td width="546"><label>
							<select name="childbirth_y" class="textarea natal_year" style="height:35px;">
								<option value="">년</option>
							<% For num=Year(Date()) To Year(DateAdd("yyyy",2,Date())) %>
								<option value="<%=num%>"><%=num%></option>
							<% Next %>
							</select>
							<select name="childbirth_m" class="textarea natal_month" style="height:35px;">
								<option value="">월</option>
							<% For num=1 To 12 %>
								<option value="<%=num%>"><%=num%>월</option>
							<% Next %>
							</select>
							<select name="childbirth_d" class="textarea natal_day" style="height:35px;">
...
<!--					<tr style="display:none;">
					...
					<tr>
						...
						<td>
							<input type="text" id="post" name="post" class="textarea" size="5" maxlength="7" valid="E|A=우편번호를 입력해 주세요"/> 
	...
	$(document).ready(function() {
		...
		$("#agree4").click(function() {
			if ($(this).is(":checked")) {
				...
				$(".agree1:eq(1)").prop("checked", false);
				$(".agree2:eq(1)").prop("checked", false);
				$(".agree3:eq(1)").prop("checked", false);
			}
		});
	});
