...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' 
  ...
  '---------------------------------------------------------------------
	...
	strTable = " FROM  MO_RESERVATION_GIFT_OUT   A WITH(NOLOCK)      "
...
<%
	If IsArray(arrList) Then 
		For num=0 To UBound(arrList,2)		
			pdt_seq		= arrList(0,num)
			out_seq		= arrList(1,num)
			out_mem_id  = arrList(2,num)
			out_date	= arrList(3,num)
			out_ea	    = arrList(4,num)

    If out_date <> "" Then 
		yyyy = Left(out_date,4)
		mm = Mid(out_date,5,2)
		dd = Right(out_date,2)
...
%>
			<tr onclick="location.href='./reservation_gift_out_ins.asp?pdt_seq=<%=pdt_seq%>&out_seq=<%=out_seq%>'" style="cursor:pointer;">
