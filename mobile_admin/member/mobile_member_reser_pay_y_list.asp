...
'---------------------------------------------------------
	...
	strWhere = " WHERE MR_CNLKBN='N'  and MR_PAY_STATE='결제완료' "
...
<body>
	<div id="container">
		<div class="linemap">마더앤베이비 > 회원관리 > <strong>입금확인 리스트</strong></div>
		<div class="write">
			<form id="sFrm" name="sFrm" method="get" action="?">
			<input type="hidden" name="brd_id" value="<%=brd_id%>">
			<input type="hidden" name="page_name" value="pay_y_list">
			<div class="selection">
				검색:
...
%>
			<tr onclick="location.href='./mobile_member_reser_ins.asp?idx=<%=idx%>&page_name=pay_y_list'" style="cursor:pointer;">
