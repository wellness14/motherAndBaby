...
<div class="wrap">
	...
	<div class="sub_contents story">
		...
		<div id="tabs">
			<div class="tab_ui">
				<ul>
					<li<% If cateCode = "" Then%> class="ov"<%End if%>><a href="#tabs-1" onclick="location.href='applist.asp?uCode=<%=AppUCODE%>';">전체</a></li>
...
Dim i,number, NickName, varCName, refererValue, chkPos, uImg
i = 0
if not(oRs.eof) Then
		...
		End If 
		 ...
		 refererValue = oRs("sSource")

		 If refererValue <> "" Then 
		 refererValue = Left(refererValue, InstrRev(refererValue, "/", -1, 1))

			If refererValue = "http://" Or refererValue = "https://" then
                   refererValue = oRs("sSource")
			End If 
...
%>
							...
							End If 
									...
									Do While Not ( Rs.eof or Rs.bof ) 
										...
										case "jpg","png", "gif", "bmp"
											Response.write "<a href=""appview.asp?uCode="&AppUCODE&"&cateCode="&cateCode&"&sNum="&oRs("sNumber")&"""><img src="""&Rs("FilePath")&"/"&Rs("filename")&""" alt=""이미지"" /></a>"
										Case "mp4"
										...
										If x = 1 Then uImg = "http://www.mothernbaby.co.kr"&Rs("FilePath")&"/"&Rs("filename")
				...
				<!-- //이미지 슬라이드 -->
					...
					<div class="story_tx">
						<a href="appview.asp?uCode=<%=AppUCODE%>&cateCode=<%=cateCode%>&sNum=<%=oRs("sNumber")%>"><%=oRs("sText")%></a>
					</div>
					<% If refererValue <> "" then%><p class="blog_link"><a href="ex<%=oRs("sSource")%>"><%=refererValue%></a></p><% End if%>
					<div class="comment_box">
						<a href="appview.asp?uCode=<%=AppUCODE%>&cateCode=<%=cateCode%>&sNum=<%=oRs("sNumber")%>"><span class="ico_reply"></span>댓글(<%=oRs("replyNum")%>)</a>
						<a href="javascript:fnshareAdd('http:///www.mothernbaby.co.kr/application/html/story/shareview.asp?sNum=<%=oRs("sNumber")%>','[맘초스토리커뮤니티]<%=NickName%>-<%=Left(oRs("sText"),20)%>','http://www.mothernbaby.co.kr/upload_application/share/150416_shareimg.jpg');"><span class="ico_share"></span>공유</a>
					</div>
				</div>
				<!-- //1set -->
<%	
...
<% end if%>
				<!-- //1set -->
				<a href="javascript:fnWriteAdd('<%=AppUCODE%>');" class="btn_write"><img src="/application/html/img/btn_write.png" alt="글쓰기" /></a>
				<!-- //스토리 플러스 -->
			...
			<div id="tabs-2" class="cate_layer">
				...
				<%   
				   ...
				   Do While Not ( oRs.eof or oRs.bof ) 
				%>
									<a href="applist.asp?uCode=<%=AppUCODE%>&cateCode=<%=oRs("category")%>"><!-- 활성화 class: sel 추가 -->
				...
				%>
				    <div class="">
						<a href="applist.asp?uCode=<%=AppUCODE%>">전체보기<em></em></a>
					</div>
				</div>
