...
'	데이터 확인
	...
	If menuCode2="C" Then urlList = "board_03.asp"
	If menuCode2="D" Then urlList = "board_04.asp"
...
'-----------------------------------------------------------------------------
	...
	Select Case mode 
		Case "ins" : 
			...
			strSql = strSql & "         BA_SEQ, BA_DPH, BA_PIDX, BA_LOCAL, BA_MEM_ID, BA_MEM_NM, BA_TITLE, "
			...
			strSql = strSql & "			'"& part &"','"& uId &"','"& mem_nm &"','"& title &"','"& content &"','"& sec_flg &"','"& intPw &"','"& user_ip &"', "
...
'			response.End 
		...
		Case "upd" : 
			...
			strSql = strSql & "			BA_LOCAL='"& part &"', BA_EMAIL='"& email &"', BA_FILE_NM='"& fileNm &"', BA_FILE_PATH='"& filePath &"', BA_UPD_DT = GETDATE() "
