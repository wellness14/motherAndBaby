...
'	reUrl = fncRequest("reUrl")
	...
	If USERID <> "" And USER_PWD <> "" Then 
		'strSql = "SELECT MM_MEM_NM, MM_MEM_TYPE FROM MO_MEM_MAS with(nolock) WHERE MM_MEM_ID='"& USERID &"' AND MM_MEM_PW = dbo.UF_MD5('"& USER_PWD &"') AND MM_DEL_DT IS NULL "
		 strSql = "SELECT UL_MEM_NM, UL_GRP_CD, UL_LEVEL FROM MO_USER_LIST with(nolock) WHERE UL_MEM_ID = '" & USERID & "' AND UL_USE_YN ='Y'  AND UL_PWD = dbo.UF_MD5('"& USER_PWD &"') "
		...
		'	End If
			...
			response.Write "<script type='text/javascript'>parent.location.href='/etc_work/mnb_admin_app/service_work.asp'</script>"
			response.End 
...
<!--
   ...
   parent.location.href="/etc_work/mnb_admin_app/login.asp";
//-->
</SCRIPT>