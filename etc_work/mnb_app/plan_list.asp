	...
	<div class="container">
		...
		<% If service_contract_check = "T" Then %>
			...
			<div class="plan-card">
				...
				<div class="plan-card-desc">
					<% If payment_dt <> "" Then %>
						계약금 : <%=dateConvert2(payment_dt)%> (<%=numberFormatConvert(payment)%>원) 입금완료<br/>
					<% Else %>
						계약금 : 결제진행중<br/>
					<% End If %>
					잔금: <%=payment_state%><br/>
		...
		<div class="plan-card">
			<div class="plan-card-title">맘앤샵 쇼핑몰</div>
			<div class="plan-card-content">맘앤샵 쇼핑몰 오픈</div>
			<div class="plan-card-line"></div>
			<div class="plan-card-desc">
				임신,출산,육아용품 쇼핑몰, 마더앤베이비 출산선물 선택, 이벤트<br/>
				엄마의 마음을 알아주는 출산준비 전문쇼핑몰 맘앤샵
			</div>
			<a href="http://www.momnshop.co.kr/m2/goods/event.php?mevent_no=1" target="_blank"><div class="plan-card-btn">쇼핑몰 가기</div></a>
		</div>
	</div>
